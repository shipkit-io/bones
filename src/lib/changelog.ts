...
// ---------------------------------------------------------------------------
// Grouping: one group per tag, plus one "Recent Updates" group for commits
// newer than the latest tag
// ---------------------------------------------------------------------------

/**
 * Slug of the untagged "Recent Updates" group.
 *
 * It must not depend on the commits it contains. An earlier version used
 * `updates-<date of newest commit>`, so every merge to main renamed the
 * group. The list page, the sitemap, the detail page and the unstable_cache
 * behind them all revalidate on their own timers, so for up to an hour they
 * disagreed on the name and /changelog linked to a slug that
 * getChangelogEntry() no longer returned (a 404 straight from the list).
 */
export const UNRELEASED_SLUG = "unreleased";

/** Matches the old `updates-YYYY-MM-DD` slugs so stale links can redirect. */
export function isLegacyUnreleasedSlug(slug: string): boolean {
	return /^updates-\d{4}-\d{2}-\d{2}$/.test(slug);
}
...
function makeGroup(commits: GitHubCommit[], tag: string | null): CommitGroup {
	return {
		label: tag ?? "Recent Updates",
		slug: tag ? tag.replace(/[^a-zA-Z0-9.-]/g, "-").toLowerCase() : UNRELEASED_SLUG,
...
function groupCommits(commits: GitHubCommit[], tagMap: Map<string, string>): CommitGroup[] {
	...
	// Walk commits newest-first, split on tags. Only the first group can be
	// untagged, so the constant UNRELEASED_SLUG is unique.
	for (const commit of commits) {
		const tag = tagMap.get(commit.sha);
		if (tag && currentGroup.length > 0) {
			groups.push(makeGroup(currentGroup, currentTag));
			currentGroup = [];
		}
...
// The cached unit throws on a GitHub failure instead of returning []. A
// rejected promise is not stored, so a rate-limited fetch (60/hour when
// GITHUB_TOKEN is unset) is retried on the next request rather than serving
// an empty list, and 404s for every entry, for the next hour.
const fetchChangelogEntries = unstable_cache(
...
export async function getChangelogEntries(): Promise<ChangelogEntry[]> {
	try {
		return await fetchChangelogEntries();
	} catch (err) {
